...
async function collectIds(browser) {
  console.log("Шаг 1: собираем ID заданий биологии...");
  ...
  page.on("response", async (response) => {
    ...
    try {
      ...
      if (!items) return;
      items.forEach(item => {
        const sid = item.themes?.[0]?.subject_id || item.task_line?.subject_id;
        if (!sid || sid === 1) taskIds.add(String(item.id));
      });
    } catch(e) {}
  });

  await page.goto("https://neofamily.ru/biologiya/task-bank", {
...
async function getTaskData(page, taskId) {
  try {
    await page.goto(`https://neofamily.ru/biologiya/task-bank/${taskId}`, {
...
async function main() {
  ...
  for (let i = 0; i < newIds.length && tasks.length < MAX_TASKS; i++) {
    ...
    let topic = "Биология", subtopic = "Общее";
    ...
    tasks.push({
      source_id: String(id),
      subject: "Биология", topic, subtopic,
