...
async function rewriteSolution(solution) {
  const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `Перефразируй это объяснение к заданию ЕГЭ по биологии своими словами, сохранив весь смысл и все факты. Не заменяй биологические термины на синонимы — оставляй их точно как в оригинале (например: консументы, продуценты, редуценты, митоз, мейоз и т.д.). Не добавляй ничего лишнего. Просто сразу пиши объяснение без вступлений. Текст:\n\n${solution}`
      }],
      max_tokens: 500,
      temperature: 0.4
    })
